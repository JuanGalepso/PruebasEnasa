...
class UserController extends Controller
{
    ...
    public function store(StoreUser $request)
    {
        ...
        //return redirect()->route('admin.usuarios.edit', ['user' => $user]);
    }
    ...
    public function edit($id)
    {
        ...
        $log = new LogSistema();
        $log->user_id = auth()->user()->id;
        $log->tx_descripcion = 'El usuario: '.auth()->user()->name.' Ha ingresado a editar los datos del usuario: '.$user->name.' a las: '
        . date('H:m:i').' del día: '.date('d/m/Y');
        $log->save();
        return view('admin.usuarios.edit', ['user' => $user]);
    }
    ...
    public function update(UpdateUser $request, $id)
    {
        ...
        if ($request->has('role'))
        {
         ...
         $log = new LogSistema();
        $log->user_id = auth()->user()->id;
        $log->tx_descripcion = 'El usuario: '.auth()->user()->name.' Ha modificó los datos del usuario: '.$user->name.' a las: '
        . date('H:m:i').' del día: '.date('d/m/Y');
        $log->save();
